...
penultimo_csv = "query_aws." + yesterday +"-20.30.csv.gz"


time.sleep(1)
print("Fazendo download do arquivo: ", penultimo_csv)
# Faz download do arquivo do S3
s3_resource.download_file(Bucket="lp-judice-csv",Key=penultimo_csv,Filename="C:\\xampp\\htdocs\\dashboard\\datalake\\ultima_query_anterior.gz")
time.sleep(1)
print("Donwload realizado com sucesso")

# Lê os dados 
df = pd.read_csv("C:\\xampp\\htdocs\\dashboard\\datalake\\ultima_query_anterior.gz", delimiter=",", encoding = "ISO-8859-1") 
...
xdata_out_dl.to_csv('C:/xampp/htdocs/dashboard/datalake/datalake-fase.csv', index=False)
print("Arquivo CSV datalake-fase criado com sucesso")
